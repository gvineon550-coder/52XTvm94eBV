...
UNSTABLE_FILE = "iptvorg_unstable.m3u"
REPORT_FILE = "iptvorg_report.txt"
STATS_FILE = "iptvorg_stats.json"
...
def is_geo_blocked(extinf):
    return "[geo" in get_name(extinf).lower() or "geo-block" in get_name(extinf).lower()
...
def main():
    ...
    try:
        ...
        send_telegram("❌ <b>IPTV-org RUS</b>\nНе удалось скачать плейлист:\n" + str(e))
        return
    ...
    for ch in all_channels:
        if get_quality(ch["extinf"]) >= MIN_QUALITY:
            ...
            dropped_quality += 1
    print("After quality: " + str(len(after_quality)) + " (dropped " + str(dropped_quality) + ")")

    after_clean = []
    dropped_mud = 0
    for ch in after_quality:
    ...
    report = []
    report.append("# IPTV-org RUS Report")
    report.append("")
    ...
    msg_lines = []
    msg_lines.append("📊 <b>IPTV-org RUS — ежедневный отчёт</b>")
    msg_lines.append("")
